...
########################################################################################
 ...
 # Load heat demand data
    ...
    if config["heating"]["enabled"]:
        hs.init_heating_sector(n, heat_demand_df, csv_file="data/heating_technologies.csv", discount_rate=0.04)
        hs.update_heating_technologies(n, year=2020, heat_demand_df=heat_demand_df)
...
############################################################
# for heating tech colors
    with open("config.yaml") as file:
        config = yaml.safe_load(file)
    
    heat_colors = config['plotting']["heat_carrier_colors"]
...
###########################################################
    ...
    for i in range(2021, 2022):
        ...
        #Updating year for heat_pump constraints according to year
        if config["heating"]["enabled"]:
            hs.update_heating_technologies(n, year=i, heat_demand_df=heat_demand_df)
        ...
        pt.pie_chart_heat_capacity(n, year=i, clusters=clusters, colors=heat_colors)
        pt.plot_heating_supply_shares(n, year=i, clusters=clusters, colors=heat_colors)
        #pt.pie_chart_electric_gen(n, year=i-1, clusters=clusters, colors=colors)
        ...
        bf.fix_efficiency2(n) ############################## added to efficiency2
        ...
        logger.info(f"Finished LOPF for year {i}")
